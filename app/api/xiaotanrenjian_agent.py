...
app = FastAPI(title="🎙️ 笑谈人间 API", version="1.0")
...
def _run(task_id: str, text: str):
    try:
        ...
        result = str(crew.kickoff(inputs={"text": text, "reference": fetch_reference("xiaotanrenjian", text)}))
...
@app.get("/xiaotanrenjian_agent/{task_id}", response_model=TaskRsp)
