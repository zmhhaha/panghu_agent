...
def llm_service_config_error() -> str | None:
    ...
    if not os.getenv("LLM_BASE_URL", "").strip():
        return "daofaziran_agent 配置不完整：LLM_BASE_URL 未注入（应指向集群内 llm-service）。"
    if not os.getenv("LLM_SERVICE_TOKEN", "").strip():
        return "daofaziran_agent 配置不完整：LLM_SERVICE_TOKEN 未注入，请检查 ExternalSecret 是否已同步。"
    return None

db.init_db("daofaziran_agent")
db.clear_stale_tasks()

app = FastAPI(title="☯️ 道法自然 API", version="3.0")
...
@app.get("/daofaziran_agent/{task_id}", response_model=TaskRsp)
