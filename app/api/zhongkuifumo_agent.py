...
def llm_service_config_error() -> str | None:
    ...
    if not os.getenv("LLM_BASE_URL", "").strip():
        return "zhongkuifumo_agent 配置不完整：LLM_BASE_URL 未注入（应指向集群内 llm-service）。"
    if not os.getenv("LLM_SERVICE_TOKEN", "").strip():
        return "zhongkuifumo_agent 配置不完整：LLM_SERVICE_TOKEN 未注入，请检查 ExternalSecret 是否已同步。"
    return None
...
def _run(task_id: str, text: str):
    try:
        ...
        result = str(crew.kickoff(inputs={"text": text, "reference": fetch_reference("zhongkuifumo", text)}))
