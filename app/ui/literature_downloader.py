"""Gradio UI for the literature downloader."""
...
def action(task_id: str, endpoint: str) -> str:
    ...
    try:
        ...
        return _render(data.get("task") or data)[:20_000]
    except requests.RequestException as exc:
        return f"操作失败：{exc}"
...
def download_file(task_id: str, endpoint: str, suffix: str) -> str:
    ...
    response = requests.get(
        f"{API_BASE}/literature-download/{task_id}/{endpoint}", timeout=60
    )
    if not response.ok:
...
with gr.Blocks(title="文献下载工具") as demo:
    ...
    report_button.click(
        lambda tid: download_file(tid, "report/download", ".md"),
        task_id,
        report_button,
    )
    pdf_button.click(
        lambda tid: download_file(tid, "files/download", ".zip"),
        task_id,
        pdf_button,
    )
