...
def do_yimaneili(text: str, request: gr.Request):
    ...
    for i in range(MAX_WAIT // 5):
        try:
            r = requests.get(f"{API_BASE}/yimaneili_agent/{task_id}", timeout=10)
...
with gr.Blocks(title="✝️ 以马内利", theme=THEME) as demo:
    gr.Markdown("# ✝️ 以马内利\n\n用圣经的眼光回你的心事。有恩典，有真理。\n\n> **以马内利，翻出来就是'神与我们同在。'**\n")
    text_input = gr.Textbox(label="", placeholder="随便写点什么…", lines=4)
    btn = gr.Button("✝️ 圣经怎么说", variant="primary", size="lg")
    output = gr.Markdown()
    btn.click(fn=do_yimaneili, inputs=[text_input], outputs=[output, btn])

if __name__ == "__main__":
    demo.queue(default_concurrency_limit=2).launch(server_name="0.0.0.0", server_port=int(os.getenv("GRADIO_SERVER_PORT", "7865")))
